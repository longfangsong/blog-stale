...
$$
我们来模拟一下匹配字符串$\bold{id}*\bold{id}+(\bold{id}+\bold{id})*\bold{id}$的过程，并尝试解释每一步选择移进还是归约的原因。

| 符号栈           | 剩余输入                                              | 动作             | 原因                                                         |
| ---------------- | ----------------------------------------------------- | ---------------- | ------------------------------------------------------------ |
|                  | $\bold{id}*\bold{id}+(\bold{id}+\bold{id})*\bold{id}$ | 移进 $\bold{id}$ | 没得选                                                       |
| $\bold{id}$      | $*\bold{id}+(\bold{id}+\bold{id})*\bold{id}$          | 栈顶归约为 $F$   | 下一个符号是$*$，而没有包含 $\bold{id} *$ 的规则，但栈顶可以归约 |
| $F$              | $*\bold{id}+(\bold{id}+\bold{id})*\bold{id}$          | 栈顶归约为 $T$   | 下一个符号是$*$，而没有包含$F *$ 的规则，但栈顶可以归约 |
| $T$              | $*\bold{id}+(\bold{id}+\bold{id})*\bold{id}$          | 移进 $*$         | 下一个符号是$*$， 而没有包含 $ T *$ 的规则              |
| $T*$             | $\bold{id}+(\bold{id}+\bold{id})*\bold{id}$           | 移进 $\bold{id}$ | 栈顶不能归约，只能移进                                       |
| $T*\bold{id}$    | $+(\bold{id}+\bold{id})*\bold{id}$                    | 栈顶归约为 $F$   | 下一个符号是$+$，而没有包含 $T *\bold{id}+$ 或者 $\bold{id}+$ 的规则，但栈顶可以归约 |
| $T*F$            | $+(\bold{id}+\bold{id})*\bold{id}$                    | 栈顶归约为 $T$   | 下一个符号是$+$，而没有包含$T * F+$ 或者 $F+$ 的规则，但栈顶可以归约 |
| $T$              | $+(\bold{id}+\bold{id})*\bold{id}$                    | 栈顶归约为 $E$   | 下一个符号是$+$，而没有包含$T+$ 的规则，但栈顶可以归约 |
| $E$              | $+(\bold{id}+\bold{id})*\bold{id}$                    | 移进 $+$         | 栈顶不能归约，只能移进                                       |
| $E+$             | $(\bold{id}+\bold{id})*\bold{id}$                     | 移进 $($         | 栈顶不能归约，只能移进                                       |
| $E+($            | $\bold{id}+\bold{id})*\bold{id}$                      | 移进 $\bold{id}$ | 栈顶不能归约，只能移进                                       |
| $E+(\bold{id}$   | $+\bold{id})*\bold{id}$                               | 栈顶归约为 $F$   | 下一个符号是$+$，而没有包含 $\bold{id}+$ 的规则，但栈顶可以归约 |
| $E+(F$           | $+\bold{id})*\bold{id}$                               | 栈顶归约为 $T$   | 下一个符号是$+$，而没有包含 $F+$ 的规则，但栈顶可以归约      |
| $E+(T$           | $+\bold{id})*\bold{id}$                               | 栈顶归约为 $E$   | 下一个符号是$+$，而没有 包含 $T+$ 的规则，但栈顶可以归约     |
| $E+(E$           | $+\bold{id})*\bold{id}$                               | 移进             | 栈顶不能归约，只能移进                                       |
| $E+(E+$          | $\bold{id})*\bold{id}$                                | 移进             | 栈顶不能归约，只能移进                                       |
| $E+(E+\bold{id}$ | $)*\bold{id}$                                         | 栈顶归约为 $F$   | 下一个符号是$)$，而没有 包含 $\bold{id})$ 的规则，但栈顶可以归约 |
| $E+(E+F$         | $)*\bold{id}$                                         | 栈顶归约为 $T$   | 下一个符号是$)$，而没有 包含 $F)$ 的规则，但栈顶可以归约     |
| $E+(E+T$         | $)*\bold{id}$                                         | 栈顶归约为 $E$   | 下一个符号是$+$，而没有包含 $T)$ 的规则，但栈顶可以归约      |
| $E+(E+E$         | $)*\bold{id}$                                         | 移进             | 栈顶不能归约，只能移进                                       |
| $E+(E+E)$        | $*\bold{id}$                                          | 栈顶归约为 $F$   | 下一个符号是$*$，没有包含 $)*$ 的规则，但栈顶可以归约        |
| $E+F$            | $*\bold{id}$                                          | 栈顶归约为 $T$             | 下一个符号是$*$，没有包含 $F*$ 的规则，但栈顶可以归约        |
| $E+T$            | $*\bold{id}$                                          | 移进             | 这里是tricky的地方，这里考虑到优先级问题必须移进，这样看来**优先移进，不行了再归约可能是处理优先级问题的方案** |
| $E+T*$          | $\bold{id}$ | 移进 | 栈顶不能归约，只能移进 |
| $E+T*\bold{id}$ |  | 栈顶归约为 $F$ | 没有符号了，往死里归约 |
...
| 状态栈         | 符号栈        | 剩余输入                | 动作                                                         |
| -------------- | ------------- | ----------------------- | ------------------------------------------------------------ |
| $I_0$          |               | $\bold{id} * \bold{id}$ | 根据状态机的标识，转到$I_5$（即压入状态栈），同时移进 $\bold{id}$ |
| $I_0I_5$       | $\bold{id}$   | $* \bold{id}$           | 根据状态机的标识，无处可转，进行归约，弹状态栈               |
| $I_0$          | $F$           | $* \bold{id}$           | 弹栈后进行下一步状态转移，转到$I_3$                          |
| $I_0I_3$       | $F$           | $* \bold{id}$           | 根据状态机的标识，无处可转，进行归约，弹状态栈               |
| $I_0$          | $T$           | $* \bold{id}$           | 弹栈后进行下一步状态转移，转到$I_2$                          |
| $I_0I_2$       | $T$           | $* \bold{id}$           | 根据状态机的标识，转到$I_7$，同时移进 $*$                    |
| $I_0I_2I_7$    | $T*$          | $\bold{id}$             | 根据状态机的标识，转到$I_5$，同时移进 $\bold{id}$            |
| $I_0I_2I_7I_5$ | $T*\bold{id}$ |                         | 根据状态机的标识，无处可转，进行归约，弹状态栈               |
